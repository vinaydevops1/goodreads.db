...
app.get("/books/:bookId/", async (request, response) => {
  ...
  const sqliteQueryBookId = `
    SELECT * FROM book 
    WHERE book_id=${bookId};`;
  const bookDetails = await db.get(sqliteQueryBookId);
...
app.get("/authors/:authorId/books/", async (request, response) => {
  ...
  const authorQuery = `SELECT * FROM book
    WHERE author_id = ${authorId};`;
  const authorList = await db.all(authorQuery);
  response.send(authorList);
});
